...
dada en string o modo texto, la gama de colores es una de grises el cual, va de 0 a 255, en este programa, se escogera
aleatoriamente los "colores" desde el color 20 al color 42 y para que estos estén en string, los convertimos en
caracter para ser analizados uno por uno en el string.
...
(define randomColor (integer->char(random 20 43))) 
  (if (<= z 1)
          (if (<= y 100)
              (if (<= x 100)
...
(AbrirGrafico2 stringCerebro 1 1 (z2) 0)
