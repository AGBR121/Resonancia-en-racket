...
#|
- Fecha:  05/05/2024
- Hora de Publicacion: 5:20pm
- Version del código: 10.0
...
- Descripcion del programa: Este programa consiste en un algoritmo para analaizar la resonancia magnetica de un paciente
dada en un vector, la gama de colores es una de grises el cual, va de 0 a 255, en este programa, se escogera
aleatoriamente los "colores" desde el color 20 al color 42.
...
#|--------------------------------------------------------------------------
Funcion Cerebro que obtiene el vector de una pagina de la resonancia
...
|#
(define (Cerebro vect z y x)
;Identificador local randomColor que escoje aleatoriamente un numero del 0 al 255
(define randomColor (random 20 43)) 
  (if (<= z 1)
          (if (<= y 100)
              (if (<= x 100)
                  (Cerebro (vector-append vect (make-vector 1 randomColor )) z y (+ 1 x))
              ;De lo contrario
                  (Cerebro vect z (+ y 1) 1)
              );Fin if (<= x 100)
         ;De lo contrario     
         (Cerebro vect (+ z 1) 1 1)
      );Fin if (<= y 100)
  ;De lo contrario
      vect
  );Fin if (<= z 1)
...
#|--------------------------------------------------------------------------
Funcion CicloCerebro que obtiene el vector de toda la resonancia
...
(define (CicloCerebro vec counter)
 (if (<= counter 100)
     (CicloCerebro (vector-append vec (Cerebro (vector) 1 1 1 )) (+ counter 1))
 ;De lo contrario
     vec
 );Fin if (<= counter 20)
...
(define vectorCerebro (CicloCerebro (vector) 1))
...
(printf "\nVamos a revisar el plano para conocer las lineas sospechosas.\n")

;Funcion z2 que guarda el valor del eje z para el segundo gráfico
(define (z2)
(printf "\nDigite el valor de la pagina o eje z: ")
;Identificador local z que recibe el valor ingresado por teclado para el eje z
(define z (- (read) 1) )
  (if (or (<= z 0) (>= z 99))
      [begin
...
(AbrirGrafico2 vectorCerebro 1 1 (z2) 0)
...
;Hacemos llamado a todas las funciones
